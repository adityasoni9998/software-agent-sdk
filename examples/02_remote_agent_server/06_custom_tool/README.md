...
3. **Using `DockerWorkspace`, `ModalWorkspace`, or `ApptainerWorkspace`** with
   the final image
...
```text
docker.io/adityasoni8/codescout-agent-server-modal-workspace:<short-sha>-codescout-modal-source-minimal
...
with ApptainerWorkspace(
    server_image="docker.io/adityasoni8/codescout-agent-server-modal-workspace:codescout-modal-source-minimal",
) as workspace:
    ...
```
...
apptainer pull codescout-agent-server.sif \
  docker://docker.io/adityasoni8/codescout-agent-server-modal-workspace:codescout-modal-source-minimal
```
...
```bash
cd examples/02_remote_agent_server/06_custom_tool
CUSTOM_AGENT_SERVER_IMAGE_TAG=docker.io/adityasoni8/codescout-agent-server-modal-workspace:codescout-modal-source-minimal \
...
To run the same CodeScout prompt and toolset in a Modal Sandbox, install the
...
Then select the Modal backend. Because this script builds the `source-minimal`
target, `ModalWorkspace` starts the source agent-server entrypoint:

```bash
CODESCOUT_WORKSPACE=modal \
CUSTOM_AGENT_SERVER_IMAGE_TAG=docker.io/adityasoni8/codescout-agent-server-modal-workspace:codescout-modal-source-minimal \
LLM_BASE_URL=... \
LLM_MODEL=... \
LLM_API_KEY=... \
...
For a private image, set `MODAL_REGISTRY_SECRET_NAME` to a Modal Secret that
contains the registry credentials.
...
with `DockerWorkspace`, `ModalWorkspace`, `ApptainerWorkspace(server_image=...)`,
